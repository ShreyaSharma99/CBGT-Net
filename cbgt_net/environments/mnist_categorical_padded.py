...
class MNISTCategoricalEnvironmentPadded(Environment):
	"""
	...
	class OneHotWrapper(Environment):
		...
		def __init__(self, base_env):
			...
			serialized_tensor_train = tf.io.read_file('cbgt_net/minst_full_train.txt')
			...
			serialized_tensor_test = tf.io.read_file('cbgt_net/minst_full_test.txt')
	...
	def __init__(self, num_categories, **kwargs):
		...
		image_shape = kwargs.get("image_shape", [28,28,3])
...
#		self._target = np.random.randint(0, self.num_categories, (self._batch_size,))
		if target is None: target = tf.transpose(self._get_random_indices(self._num_categories))
