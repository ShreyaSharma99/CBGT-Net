...
class Miniworld_bldg_fire_CategoricalEnvironment(Environment):
	"""
	...
	class OneHotWrapper(Environment):
		...
		def __init__(self, base_env):
			...
			no_fire_tensor = tf.io.read_file('cbgt_net/no_fire.txt')
			# self._image_data_train = tf.io.parse_tensor(serialized_tensor_train, out_type=tf.float32)[..., :1]
			no_fire_tensor = tf.io.parse_tensor(no_fire_tensor, out_type=tf.uint8)

			fire_tensor = tf.io.read_file('cbgt_net/fire.txt')
			fire_tensor = tf.io.parse_tensor(fire_tensor, out_type=tf.uint8)
			self._image_data_train = tf.stack([no_fire_tensor, fire_tensor], axis=0)

			no_fire_tensor_test = tf.io.read_file('cbgt_net/no_fire_test.txt')
			no_fire_tensor_test = tf.io.parse_tensor(no_fire_tensor_test, out_type=tf.uint8)

			fire_tensor_test = tf.io.read_file('cbgt_net/fire_test.txt')
			fire_tensor_test = tf.io.parse_tensor(fire_tensor_test, out_type=tf.uint8)
			self._image_data_test = tf.stack([no_fire_tensor_test, fire_tensor_test], axis=0)


			# self._data_size = self._fire_tensor.shape[1]
		...
		def observe(self, observation_mode=None, training=True, time_step=None):
			...
			if training:
				# print("type(self._image_data_train) ", type(self._image_data_train))
				# images = self._image_data_train[target_val, target_color, img_index]
				...
				# self._image_data_test[target_val, target_color, img_index]
	...
	def __init__(self, num_categories, **kwargs):
		...
		image_shape = kwargs.get("image_shape", [60,80,3])
...
#		self._non_target_categories = np.stack([np.delete(np.arange(self.num_categories),t) for t in self._target])
	...
	def observe(self, observation_mode=None, training=True, time_step=None):
		...
		# tf.print("target_val - ", self.target_index[:10])
		# Increment the observation count
		self._num_observations.assign(self._num_observations + 1)
		target_val = tf.convert_to_tensor(self.target_index, dtype=tf.dtypes.int32)
		target_color = tf.random.uniform([self._batch_size], minval=0, maxval=num_colors, dtype=tf.dtypes.int32)
		...
		img_index = tf.convert_to_tensor(self.img_index, dtype=tf.dtypes.int32)
		# img_index = tf.tile(img_index, [1, self._patch_size[0], self._patch_size[1]])
		...
		indices = tf.stack([target_val, target_color, img_index], axis=-1)
