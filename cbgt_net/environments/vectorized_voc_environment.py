...
class Vectorized_VOC_ObjDetect(Environment):
	"""
	...
	def __init__(self, categories, **kwargs):
		...
		self._voc_data = tfds.as_numpy(tfds.load(
						'voc',
						split='train',
						batch_size=-1,
						))

		self._num_datapoints = self._voc_data["image"].shape[0]
		...
		for i in range(self._voc_data["image"].shape[0]):
			### DANA:  Why the deep copy?
			box_list_copy = copy.deepcopy(self._voc_data["objects"]["bbox"][i])
			bb_count = np.argmin(np.sum(box_list_copy, axis=1))
			for j in range(bb_count):
				label_value = self._voc_data["objects"]["label"][i][j]
				if label_value in allowed_classes.keys():
					self._limited_ind += ([(i, j)] * class_wts[label_value])

		self._limited_ind = np.array(self._limited_ind)

		# Pre-compute the image sizes
		self._all_image_dims = self.image_size_vector(self._voc_data["image"])
...
###		self._voc_data = voc_data
		...
		self.reset(kwargs.get("data_index",None), self._batch_size)
	...
	def reset(self, data_index=None, batch_size=None):
		"""
		...
		### DANA:  I'm not sure whay this is here, but it's an expensive operation!
###		voc_data = copy.deepcopy(self.voc_data)
		...
		# If a batch size is provided, set the object's batch size to that.
		# Otherwise, use the interally stored batch size
		...
		if data_index is None:
			self._data_index = np.random.randint(0, self.num_datapoints, (self._batch_size,))
		...
		self._image = self.voc_data["image"][self._image_bbox_pair[:,0]]
		self._filename = self.voc_data["image/filename"][self._image_bbox_pair[:,0]]
		self._labels = self.voc_data["labels"][self._image_bbox_pair[:,0]]
		self._labels_no_difficult = self.voc_data["labels_no_difficult"][self._image_bbox_pair[:,0]]
		### DANA:  Why modify the voc_data contents and not just make a new dictionary?
#		self._objects = self.voc_data["objects"]
		self._objects = {}
		...
		self._objects["bbox"] = self.voc_data["objects"]["bbox"][self._image_bbox_pair[:,0], self._image_bbox_pair[:,1]]
		self._objects["is_difficult"] = self.voc_data["objects"]["is_difficult"][self._image_bbox_pair[:,0], self._image_bbox_pair[:,1]]
		self._objects["is_truncated"] = self.voc_data["objects"]["is_truncated"][self._image_bbox_pair[:,0], self._image_bbox_pair[:,1]]
		self._objects["label"] = self.voc_data["objects"]["label"][self._image_bbox_pair[:,0], self._image_bbox_pair[:,1]]
		self._objects["pose"] = self.voc_data["objects"]["pose"][self._image_bbox_pair[:,0], self._image_bbox_pair[:,1]]

		self._image_dim = self._all_image_dims[self._image_bbox_pair[:,0]]

#		self._image_dim = np.array([self.image_size(img) for img in self._image])
#		self._image_dim = self.image_size_vector(self._image)

		allowed_classes = {6:0, 8:1, 14:2}
		if allowed_classes is not None:
			self._target_index = np.array([allowed_classes[x] for x in self.objects["label"]])
		else:
		...
		## This is moved from observation, since 1) is a bit of an expensive
		## calculation, and 2) only needs to be done once for each image
		pad_x, pad_y = int(self._patch_size[1]/2), int(self._patch_size[0]/2)
		# print("Noise - ", self._noise)
		self._image_padded = np.pad(self._image, [(0,0), (pad_y, pad_y), (pad_x, pad_x), (0, 0)], mode='constant')
	...
	def image_size_vector(self, images):

		# Reduce the color channel
		image_vals = np.sum(images, axis=3)

		# Sum up the image values along each axis, to indicate which rows and
		# columns have non-black pixels
		image_sum_x = np.sum(image_vals, axis=1)
		image_sum_y = np.sum(image_vals, axis=2)

		# Create an array (initialized to 0) to store the widths and heights
		image_width = np.zeros((images.shape[0],), dtype=np.int32)
		image_height = np.zeros((images.shape[0],), dtype=np.int32)

		# Iterate through each row / column of the images, and update the widths
		# and heights if there's a non-zero row / column
		for idx in range(image_sum_x.shape[1]):
			image_width[image_sum_x[:,idx] > 0] = idx
		for idx in range(image_sum_y.shape[1]):
			image_height[image_sum_y[:,idx] > 0] = idx

		image_width += 1
		image_height += 1

		return np.array([image_height, image_width]).T


	def image_size(self, img):

		img_vals = np.sum(img, axis=2)
		img_width = np.max(np.where(np.sum(img_vals, axis=0)>0))
		img_height = np.max(np.where(np.sum(img_vals, axis=1)>0))

		return img_height+1, img_width+1
...
###		pad_x, pad_y = int(self._patch_size[1]/2), int(self._patch_size[0]/2)
		# print("Noise - ", self._noise)
###		image_padded = np.pad(self._image, [(0,0), (pad_y, pad_y), (pad_x, pad_x), (0, 0)], mode='constant')
		...
		### DANA:  I HAVE TAKEN A BREAK HERE ###
...
#		out_y = np.hstack((np.arange(0, y1), np.arange(y2, image_h)))
		...
		num_out_pixels_x = x1 + image_w - x2
		num_out_pixels_y = y1 + image_h - y2

		r_num = np.random.random((self._batch_size,))
		# Interesting side effect:  adding two boolean vectors in numpy is equivalent to or!
		no_in_points = (x2 - x1 < 1) + (y2 - y1 < 1)
		no_out_points = (num_out_pixels_x==0) + (num_out_pixels_y==0)

		if np.any(no_in_points) and np.any(no_out_points):
			print("No in or out points for the given patch and box size", self.patch_size, " box coord = " ,y1, x1, y2, x2)

		# Default to getting a patch in the image
		r_x = np.random.randint(x1, x2)
		r_y = np.random.randint(y1, y2)

		# Determine which indices need to be distractors, and reset the r_x and
		# r_y values accordingly
		distractor_indices = (1-no_out_points)*((r_num < self._noise) + no_in_points)
		distractor_x = np.random.randint(num_out_pixels_x)
		distractor_x = distractor_x + (distractor_x > x1)*(x2-x1)
		distractor_y = np.random.randint(num_out_pixels_y)
		distractor_y = distractor_y + (distractor_y > y1)*(y2-y1)

		r_x[distractor_indices] = distractor_x
		r_y[distractor_indices] = distractor_y

		# Lump these indices into tuples
		patch_index = np.vstack([r_x, r_y]).T
...
#		img_patch = np.array(image_padded[patch_index[1] : patch_index[1] + self._patch_size[1], patch_index[0] : patch_index[0] + self._patch_size[0], :])
		# DANA:  There's probably a better way to do this than list iteration, but
		#        I need to look up the numpy function
		img_patch = np.array([self._image_padded[i, r_y[i]:r_y[i]+self._patch_size[1], r_x[i]:r_x[i]+self._patch_size[0], :] for i in range(self._image_padded.shape[0])])
