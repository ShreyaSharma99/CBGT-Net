...
class MNIST_LargeData_CategoricalEnvironment_Vectorized_Patch(Environment):
	"""
	...
	def reset(self, target=None, batch_size=None, patch_size=None, training=True):
		...
		# print("self._max_steps_per_episode - ", self._max_steps_per_episode)
		# tf.random.uniform([self._batch_size], minval=0, maxval=self.num_categories, dtype=tf.dtypes.int32)
		# print("tf.convert_to_tensor(self._target_value) - ", tf.convert_to_tensor(self._target_value).shape)
		target_val = tf.reshape(tf.convert_to_tensor(self._target_value, dtype=tf.dtypes.int32), (-1,1,1,1))
		...
		if training:
			self._observed_patches = tf.gather_nd(self.image_data_train, indices)
			# [:, :, :, :, 0]
		else:
			self._observed_patches = tf.gather_nd(self.image_data_test, indices)

		# target_val_t = tf.reshape(tf.random.uniform([self._batch_size], minval=0, maxval=self.num_categories, dtype=tf.dtypes.int32), (-1,1,1,1))
		# target_val_t = tf.tile(target_val_t, [1, self._max_steps_per_episode, self._patch_size[0], self._patch_size[1]])

		# img_index_t = tf.reshape(tf.random.uniform([self._batch_size], minval=0, maxval=self._images_per_class, dtype=tf.dtypes.int32), (-1,1,1,1))
		# img_index_t = tf.tile(img_index_t, [1, self._max_steps_per_episode, self._patch_size[0], self._patch_size[1]])

		# x_patch_t = tf.repeat(tf.random.uniform([self._batch_size, self._max_steps_per_episode, 1], minval=0, maxval=self._image_shape[0]-self._patch_size[0]+1, dtype=tf.dtypes.int32), repeats=self._patch_size[0], axis=-1)
		# y_patch_t = tf.repeat(tf.random.uniform([self._batch_size, self._max_steps_per_episode, 1], minval=0, maxval=self._image_shape[1]-self._patch_size[1]+1, dtype=tf.dtypes.int32), repeats=self._patch_size[1], axis=-1)

		# offset_x_t = tf.range(self._patch_size[0], delta=1, dtype=tf.dtypes.int32, name='range')
		# offset_x_repeat_t = tf.repeat(tf.expand_dims(tf.expand_dims(offset_x_t, axis=0), axis=0), repeats=self._max_steps_per_episode, axis=1)
		# offset_x_repeat_t = tf.repeat(offset_x_repeat_t, repeats=self._batch_size, axis=0)
		# x_patch_indices_t = tf.repeat(tf.expand_dims(x_patch_t + offset_x_repeat_t, axis=3), repeats=self._patch_size[1], axis=3)

		# offset_y_t = tf.range(self._patch_size[1], delta=1, dtype=tf.dtypes.int32, name='range')
		# offset_y_repeat_t = tf.repeat(tf.expand_dims(tf.expand_dims(offset_y_t, axis=0), axis=0), repeats=self._max_steps_per_episode, axis=1)
		# offset_y_repeat_t = tf.repeat(offset_y_repeat_t, repeats=self._batch_size, axis=0)
		# y_patch_indices_t = tf.repeat(tf.expand_dims(y_patch_t + offset_y_repeat_t, axis=2), repeats=self._patch_size[0], axis=2)

		# indices_t = tf.stack([target_val_t, img_index_t, x_patch_indices_t, y_patch_indices_t], axis=-1)
		# self._observed_patches_test = tf.gather_nd(self.image_data_test, indices)

		# print("Shape _observed_patches_train - ", self._observed_patches_train.shape)
		# print("Shape _observed_patches_test - ", self._observed_patches_test.shape)
	...
	def observe(self, training=True, observation_mode=None, time_step=None):
		...
		# plt.imshow(self._observed_patches[10, time_step, :, :], cmap='gray') 
		# plt.axis('off')  # Turn off the axis
		# plt.savefig('cbgt_net/dump_mnist_patch/' + str(training) + "_" + str(time_step) + '.png', bbox_inches='tight', pad_inches=0)
		# print("self._observed_patches.shape - ", tf.expand_dims(self._observed_patches[:, time_step, :, :], axis=3).shape)
		# return tf.expand_dims(self._observed_patches[:, time_step, :, :], axis=3)
		return self._observed_patches[:, time_step, :, :, :]

		# if training:
		# 	# plt.imshow(self._observed_patches_train[10, time_step, :, :], cmap='gray') 
		# 	# plt.axis('off')  # Turn off the axis
		# 	# plt.savefig('cbgt_net/dump_mnist_patch/train_' + str(time_step) + '.png', bbox_inches='tight', pad_inches=0)
		# 	return self._observed_patches_train[:, time_step, :, :]

		# else:
		# 	plt.imshow(self._observed_patches_test[10, time_step, :, :], cmap='gray') 
		# 	plt.axis('off')  # Turn off the axis
		# 	plt.savefig('cbgt_net/dump_mnist_patch/test_' + str(time_step) + '.png', bbox_inches='tight', pad_inches=0)
		# 	return self._observed_patches_test[:, time_step, :, :]
	...
	# 		num_data_per_class = 50
	# 		data_index = np.random.randint(0, num_data_per_class, (self._batch_size,))
	# 		observation = np.copy(self.target_value)
