...
class CIFAR_CategoricalEnvironment_Parallel(Environment):
	"""
	...
	def __init__(self, num_categories, **kwargs):
		...
		max_steps_per_episode = kwargs.get("max_steps_per_episode", 10)
		stack_size = self._batch_size * max_steps_per_episode
		...
		self._max_steps_per_expisode = tf.constant(max_steps_per_episode, dtype=tf.int32)
		self._stack_size = tf.constant(stack_size, dtype=tf.int32)
	...
	@tf.function
	def _get_random_indices(self, max_idx, size_batch):
		"""
		...
		return tf.random.categorical(logits, size_batch, dtype=tf.int32)
...
#		self._target = np.random.randint(0, self.num_categories, (self._batch_size,))
		if target is None:
			target = tf.transpose(self._get_random_indices(self._num_categories, self._batch_size)) 
		images_per_class = self._images_per_class_train if training else self._images_per_class_test
		img_index = tf.transpose(self._get_random_indices(images_per_class, self._batch_size)) 
...
#		self._non_target_categories = np.stack([np.delete(np.arange(self.num_categories),t) for t in self._target])
	...
	def observe(self, observation_mode=None, training=True, time_step=None):
		...
		target_val = tf.tile(target_val, [self._max_steps_per_expisode, self._patch_size[0], self._patch_size[1]]) # blocks of batches stacked together vertically
		...
		img_index = tf.tile(img_index, [self._max_steps_per_expisode, self._patch_size[0], self._patch_size[1]])
		# tf.print("img_index - ", img_index.shape)

		x_patch = tf.repeat(tf.random.uniform([self._stack_size, 1], minval=0, maxval=self._image_shape[0]-self._patch_size[0]+1, dtype=tf.dtypes.int32), repeats=self._patch_size[0], axis=-1)
		y_patch = tf.repeat(tf.random.uniform([self._stack_size, 1], minval=0, maxval=self._image_shape[1]-self._patch_size[1]+1, dtype=tf.dtypes.int32), repeats=self._patch_size[1], axis=-1)
		...
		offset_x_repeat = tf.repeat(tf.expand_dims(offset_x, axis=0), repeats=self._stack_size, axis=0)
		...
		offset_y_repeat = tf.repeat(tf.expand_dims(offset_y, axis=0), repeats=self._stack_size, axis=0)
