...
class CIFAR_CategoricalEnvironment_CustomPad(Environment):
	"""
	...
	class OneHotWrapper(Environment):
		...
		def reset(self, target=None, training=True):
			...
			self._base_env.reset(target, training)
		...
		def observe(self, observation_mode=None, training=True, time_step=None):
			...
			"""
			observations_idx, white_tensor, white_tensor_idx = self._base_env.observe(observation_mode, training=training)
			
			if training:
				images = tf.cast(tf.gather_nd(self._image_data_train, observations_idx), tf.float32)/255.0
			else:
				images = tf.cast(tf.gather_nd(self._image_data_test, observations_idx), tf.float32)/255.0	

			images_flat = tf.reshape(images, (-1,))
			modified_tensor = tf.tensor_scatter_nd_update(white_tensor, white_tensor_idx, images_flat)


			#######
			
			# # padded_images = tf.pad(images, self._paddings, "CONSTANT")
			# padded_images = tf.pad(images, custom_padding, "CONSTANT")
			# #######
			print("padded_modified images shape = ", modified_tensor.shape)
...
#		self._non_target_categories = np.stack([np.delete(np.arange(self.num_categories),t) for t in self._target])
	...
	def observe(self, observation_mode=None, training=True, time_step=None):
		...
		x_idx = tf.random.uniform([self._batch_size, 1], minval=0, maxval=self._image_shape[0]-self._patch_size[0]+1, dtype=tf.dtypes.int32)
		y_idx = tf.random.uniform([self._batch_size, 1], minval=0, maxval=self._image_shape[1]-self._patch_size[1]+1, dtype=tf.dtypes.int32)

		# tf.print("x_idx = ", x_idx[:10])
		# tf.print("y_idx = ", y_idx[:10])

		top_padding = x_idx
		bottom_padding = -(x_idx + self._patch_size[0] - self._image_shape[0]) # self._image_shape[0] - self._patch_size[0] - x_idx
		left_padding = y_idx
		right_padding = -(y_idx + self._patch_size[1] - self._image_shape[1])
		# custom_padding = tf.constant([[0, 0], [top_padding, bottom_padding], [left_padding, right_padding], [0, 0]], dtype=tf.int32)


		x_patch = tf.repeat(x_idx, repeats=self._patch_size[0], axis=-1)
		y_patch = tf.repeat(y_idx, repeats=self._patch_size[1], axis=-1)
		...
		white_x_patch_indices = tf.repeat(tf.expand_dims(x_patch_indices, axis=3), repeats=self._patch_size[2], axis=3)
		...
		white_y_patch_indices = tf.repeat(tf.expand_dims(y_patch_indices, axis=3), repeats=self._patch_size[2], axis=3)

		indices = tf.stack([target_val, img_index, x_patch_indices, y_patch_indices], axis=-1)

		white_tensor = tf.fill((self._batch_size, self._image_shape[0], self._image_shape[1], self._image_shape[2]), 1.0, name='white_tensor')
		white_tensor = tf.cast(white_tensor, dtype=tf.float32)

		b_idx = tf.range(0, self._batch_size, 1)
		b_idx = tf.reshape(tf.convert_to_tensor(b_idx, dtype=tf.dtypes.int32), (-1,1,1,1))
		b_idx = tf.tile(b_idx, [1, self._patch_size[0], self._patch_size[1], self._patch_size[2]])

		# print()
		channel_idx = tf.range(0, 3, 1)
		channel_idx = tf.reshape(tf.convert_to_tensor(channel_idx, dtype=tf.dtypes.int32), (1,1,1, -1))
		# print("channel idx sahpe = ", channel_idx.shape)
		channel_idx = tf.tile(channel_idx, [self._batch_size, self._patch_size[0], self._patch_size[1], 1])

		# print("channel idx sahpe = ", channel_idx.shape)
		white_tensor_idx = tf.stack([b_idx, white_x_patch_indices, white_y_patch_indices, channel_idx])
		white_tensor_idx = tf.reshape(white_tensor_idx, (4, -1))
		white_tensor_idx = tf.transpose(white_tensor_idx) # shape = (batch*patch_0*patch_1*patch_2, 4)

		print("white_tensor_idx shape - ", white_tensor_idx.shape)
		# 4096, 32, 32, 4
		# tf.print(img_index)
		return indices, white_tensor, white_tensor_idx
