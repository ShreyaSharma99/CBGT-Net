...
class CE_Loss:
	...
	def __call__(self, episode, model=None):
		...
		indices = tf.concat([time_idx, batch_idx], 2)

		y_true_label = tf.squeeze(tf.cast(episode.targets, dtype=tf.int32))
	
		y_true = tf.one_hot(y_true_label, depth=10)
		choice_probabilities = tf.gather_nd(episode.decision_probabilities, indices)

		# Calculate the BCE loss between the scalar true class value and predicted probabilities
		bce_loss = tf.keras.losses.BinaryCrossentropy(from_logits=False)(y_true, choice_probabilities)
		return LossBatch(bce_loss)
