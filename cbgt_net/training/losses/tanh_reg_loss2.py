...
class TanhRegularLoss2:
	...
	def __call__(self, episode, model=None, attribute_name="losses"):
		...
		attribute = episode.evidence # max_time_step x batch_size x size_of_output
		...
		tanh_reg_loss = tf.reduce_sum(tf.reduce_mean(tf.reduce_mean(attribute, axis=1), axis=0))  # taking a mean along batch size 
		# tf.print(tanh_reg_loss)
		# tf.print(attribute)
