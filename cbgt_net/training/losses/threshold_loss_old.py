...
An implementation of the loss function for learning dynamic threshold value. 
"""

import tensorflow as tf

class ThresholdLossOld:
	"""
	The ThresholdLoss class describes a function used by the threshold learning
	algorithm. In essence, the loss function is the negative log likelihood
	of the threshold probabilities times the return of the choice.
	"""
	...
	def __call__(self, episode, model=None, attribute_name="losses"):
		...
		did_choose_distribution = episode.threshold_vector
		...
		if self._time_idx is None or self._batch_idx is None or \
		   self._timesteps != did_choose_distribution.shape[0] or \
		   self._num_batches != did_choose_distribution.shape[1]:
			# Get the dimensions needed from the choice distribution, and 
			# create the indices
			self._timesteps = did_choose_distribution.shape[0]
			self._num_batches = did_choose_distribution.shape[1]
			self.__create_indices()
		...
		did_choose_probabilities = tf.gather_nd(did_choose_distribution, indices)
		did_choose_probabilities = tf.reshape(did_choose_probabilities, did_choose_probabilities.shape + [1])
		did_choose_probabilities = tf.math.log(did_choose_probabilities+1e-32)
		did_choose_loss = -tf.math.reduce_sum(did_choose_probabilities * episode.returns_threshold, [0,2])

		# Add the reward to the episode, if an attribute name has been provided
		if attribute_name is not None and attribute_name != "":
			episode.add_attribute(attribute_name, did_choose_loss)
			

		return did_choose_loss
