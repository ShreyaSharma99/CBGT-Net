...
An implementation of the loss function used by Threshold module. 
"""
...
class ThresholdLoss:
	...
	def __call__(self, episode, model=None):
		...
		# print("Threshold resturn - ", returns)
		# print("Episode attr ", episode.attributes.keys())
		# print("Indices shape = ", indices.shape, " episode.did_decide_probabilities shape = ", episode.did_decide_probabilities.shape)
		# Get the log probability of each choice taken
		thresh_probabilities = tf.gather_nd(episode.attributes["threshold_vectors"], indices)
		thresh_probabilities = tf.expand_dims(thresh_probabilities, -1)
		thresh_loss = -tf.math.reduce_sum(tf.math.log(thresh_probabilities) * returns, [0,2])

		# tf.print("Threshold loss = ", episode.did_decide_probabilities.shape)
		return LossBatch(thresh_loss)
