...
class SimpleAccumulatorModule(keras.layers.Layer):
    """
    ...
    def __init__(self, num_categories, **kwargs):
        ...
        super(SimpleAccumulatorModule, self).__init__()
        ...
        if num_categories <= 0:
            self.logger.error("%s: Number of categories must be greater than 0: %d", self, num_categories)
            raise ValueError("Non-positive number of categories: %d" % num_categories)

        self._num_categories = tf.constant(num_categories, dtype=tf.int32)
    ...
    def reset(self, batch_size=None):
        ...
        self._accumulator.assign(np.zeros((self._batch_size, self._num_categories)))
...
###            accumulated_evidence = evidence + self._accumulator.value()
